...
# 1. ഇവന്റ് വിവരങ്ങൾ ഫോമിലേക്ക് ലോഡ് ചെയ്യുന്ന GET റൂട്ട്
@app.get("/events/{event_id}/edit", response_class=HTMLResponse)
async def edit_event_page(request: Request, event_id: int):
    if request.cookies.get("admin_session") != "authenticated":
        return RedirectResponse(url="/admin", status_code=303)
...
# 2. അപ്ഡേറ്റ് ചെയ്ത വിവരങ്ങൾ സേവ് ചെയ്യുന്ന POST റൂട്ട്
...
async def update_event(
    ...
    title: str = Form(...),
    event_date: str = Form(...),
    category: str = Form("Workshop / Seminar"),
    start_time: str = Form(""),
    end_time: str = Form(""),
    venue: str = Form(""),
    ...
    if conn:
        try:
            ...
            cur.execute("""
                UPDATE events 
                SET title = %s, event_date = %s, category = %s, 
                    start_time = %s, end_time = %s, venue = %s, 
                    max_capacity = %s, description = %s
                WHERE id = %s;
            """, (title, event_date, category, start_time, end_time, venue, max_capacity, description, event_id))
            conn.commit()
            cur.close()
        except Exception as e:
